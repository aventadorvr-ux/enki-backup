...
export default function PropertyDetailPage() {
  const [property, setProperty] = useState(baseProperty);
    ...
    async function fetchIntel() {
      try {
        ...
        setProperty((prev) => ({
          ...prev,
          aiInsights: {
            estimate: intel.median_price ?? prev.aiInsights.estimate,
            confidence: prev.aiInsights.confidence,
            trend: intel.trend === "rising" ? "up" : intel.trend === "falling" ? "down" : "up",
            trendValue:
              typeof intel.price_change === "number"
                ? `${intel.price_change > 0 ? "+" : ""}${intel.price_change}% market change`
                : prev.aiInsights.trendValue,
          },
          description: intel.ai_insights
            ? `${prev.description}\n\nMarket Intelligence:\n${intel.ai_insights}`
            : prev.description,
        }));
      } catch (e) {
