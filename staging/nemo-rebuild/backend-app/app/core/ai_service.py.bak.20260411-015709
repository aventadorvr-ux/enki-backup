...
class AIService:
    ...
    def generate(self, prompt: str, model: str = "openrouter/moonshotai/kimi-k2.5", 
    ...
    def generate_property_description(self, bedrooms: int, bathrooms: int, location: str,
        ...
        if result and len(result.split()) > 50:
            ...
            bedroom_ok = (
                ...
                f"{bedrooms}-bedroom" in result_lower or
                f"{bedrooms} bedrooms" in result_lower or
                f"{bedrooms}-bedrooms" in result_lower
            )
            bathroom_ok = (
                f"{bathrooms} bathroom" in result_lower or
                f"{bathrooms}-bathroom" in result_lower or
...
def _predict_sale_price(self, bedrooms: int, bathrooms: int, suburb: str,
                        land_size: float = None, condition: str = "good") -> dict:
    price = 800000 + max(0, bedrooms - 3) * 75000 + max(0, bathrooms - 2) * 50000
    if land_size:
        try:
            price += min(max(float(land_size), 0), 1200) * 350
        except Exception:
            pass
    price += {"poor": -120000, "fair": -60000, "good": 0, "very good": 40000, "excellent": 80000, "new": 100000}.get(str(condition).strip().lower(), 0)
    return {
        "estimated_price": int(price),
        "price_range": {"low": int(price * 0.95), "high": int(price * 1.05)},
        "confidence": "medium" if suburb else "low",
        "market_days_estimate": "30-45 days",
        "reasoning": f"Heuristic estimate for {suburb or 'unknown suburb'}",
        "method": "fallback_stub",
    }

def _analyze_market(self, suburb: str) -> dict:
    return {
        "analysis": f"{suburb or 'This suburb'} shows balanced conditions with typical buyer demand and moderate stock levels.",
        "trend": "stable",
...
AIService.predict_sale_price = _predict_sale_price
AIService.analyze_market = _analyze_market
